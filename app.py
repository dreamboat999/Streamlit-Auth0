...
def main():
    ...
    def _launch_apps():
        messageboard.empty()
        st.write('Hello,', st.session_state.user, '! :hand:')
    ...
    if st.checkbox('More info', False):
        st.write('Token:', st.session_state.token)
        st.write('User:', st.session_state.user)
        st.write('Email:', st.session_state.email)
    
    # ABOUT
    st.sidebar.title('Streamlit Auth0 Template')
    st.sidebar.info('Streamlit application integrated Auth0 user indentity and API access authentication.\n\n')
    st.sidebar.markdown('---')
